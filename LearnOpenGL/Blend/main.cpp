...
int init() {
	...
	currentWindow = glfwCreateWindow(w_width, w_height, "Blend Test", nullptr, nullptr);
...
int main(void) {
	init();
	//blend shaders
	grassImage = loadTexture("Images/grass.png");
	Shader blendShader("Shaders/blending.vert", "Shaders/blending.frag");
	...
	blinnLightShader.setInt("texture_diffuse1", 0);
...
//	stbi_set_flip_vertically_on_load(true);
	//you must enable this because of the gl bind texCoords it sets to 2. ALWAYS goona be 2.
	...
	while (!glfwWindowShouldClose(currentWindow)) {
		...
		model = scale(model, vec3(0.5f, 0.5f, 0.5f)); // Scale it down.
		...
		glBindVertexArray(transparentVAO);
		glBindTexture(GL_TEXTURE_2D, grassImage);
		for (unsigned int i = 0; i < vegetation.size(); i++)
		{
			model2 = mat4(1.0f);
			model2 = translate(model2, vegetation[i]);
			blendShader.setMat4("model", model2);
	...
	//model = translate(model, vec3(0.0f, -1.6f, 0.0f));
	//shaderSingleColor.setMat4("model", model);
	//plane.Draw(shaderSingleColor);
